...
namespace Site_MVC_FinTech.Models
{
    public class Repositorio
    {
        ...
        /*FILTRAR   */
        public static IEnumerable<Investidor> FiltrarPacote()
        {
            return FiltrarPacote(null, null);
        }

        public static IEnumerable<Investidor> FiltrarPacote(string texto, string combo)
        {
            ...
            using (var ctx = new ClassContext())
            {
                ...
                switch (combo)
                {
                    case "pacotes":
                        if (texto != "")
                            investidor = ctx.Investidor.Where(f => f.pacote.Descricao.Contains(texto)).OrderBy(x => x.pacote.Descricao).ToList();
                        break;

                    default:
                        investidor = ctx.Investidor.OrderBy(x => x.pessoa.IDPessoa).ToList();
                        break;
                }
